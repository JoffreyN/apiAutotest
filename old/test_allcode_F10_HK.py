...
pathList=[
	'/doraemon/hkf10/announcements',
	'/doraemon/hkf10/brief',
	'/doraemon/hkf10/finance',
	'/doraemon/hkf10/myAnnouncements',
	'/doraemon/hkf10/newsList',
	'/doraemon/hkf10/sellShort',
]
#########################################http###################################################################
def test_F10_HK(stockCode,path):
	stockCode=f'E{stockCode}'
	if path=='/doraemon/hkf10/myAnnouncements':
		dataJson={'marketAndCodes':stockCode}
	else:
	...
	sql=f"INSERT INTO interfaceTest_data.f10_HK_2 (created_at,path,req_data,success,failed_reason,elapsed,resp_data) VALUES ('{created_at}','{path}','{formatSqlStr(dataJson)}',{success},'{formatSqlStr(reason)}',{elapsed},'{formatSqlStr(respJson)}');"
	saveTodatabase(sql)
...
if __name__ == '__main__':
	...
	def startTest(path):
		for code in allCode:
			try:
				test_F10_HK(code[0],path)
			except:
				logging.error(f'{code[0]} {path} 发生异常:',exc_info=True)
