...
logging.getLogger().addHandler(HandlerFactory.get_rotating_file_handler('test_allcode_F10_HK_finance.Log'))
...
#########################################http###################################################################
def test_F10_HK_finance(stockCode):
	stockCode=f'E{stockCode}'
	path='/doraemon/hkf10/finance'
	...
	sql=f"INSERT INTO interfaceTest_data.f10_HK_finance (created_at,path,req_data,success,failed_reason,elapsed,resp_data) VALUES ('{created_at}','{path}','{formatSqlStr(dataJson)}',{success},'{formatSqlStr(reason)}',{elapsed},'{formatSqlStr(respJson)}');"
	saveTodatabase(sql)
